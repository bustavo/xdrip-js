...
// Adding support for Firefly g6 Transmitters.
//
// To set default timers of 10-day sensor session & 2-hour warmup ( 0xF040 )
const ResetTxFireflyDefaultMessage = require('./messages/firefly-reset-default-tx-message');
// To set extended timers of 60-day sensor session & 50-minute warmup ( 0xF080 )
const ResetTxFireflyExtendedMessage = require('./messages/firefly-reset-extended-tx-message');
// To retrieve the current patch & timer status of a transmitter ( 0x3B )
const FireflyTxStatusMessage = require('./messages/firefly-status-tx-message');
//
...
async function sendCommand(command, activationDate) {
  ...
  switch (command.type) {
    ...
    case 'ResetTxFireflyDefault': {
      txMessage = new ResetTxFireflyDefaultMessage();
      break;
    }
    case 'ResetTxFireflyExtended': {
      txMessage = new ResetTxFireflyExtendedMessage();
      break;
    }
    case 'FireflyStatus': {
      txMessage = new FireflyTxStatusMessage();
      break;
    }
  default:
      // do nothing
